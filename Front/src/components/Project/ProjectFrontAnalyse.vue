...
const calculateAverageScores = (reports: Report[]) => {
  ...
  scoreSums.forEach((sum, key) => {
    averageScores.set(key, sum / (scoreCounts.get(key) || 0));
  });
...
<template>
          ...
          <template v-for="(key, keyNumber) in Object.keys(report.report)">
            <div v-if="detail===-1" @click="detailOpen(keyNumber)" class="grid">
              <div style="display: flex; gap:7px; flex: 1">
                ...
                <h2 style="text-align: left; margin-top: 5px">점수:{{ (report.report[key].score * 100) }}</h2>

              </div>
              <svg fill="none" height="20" style="margin-top: 5px;" viewBox="0 0 12 20" width="20" xmlns="http://www.w3.org/2000/svg" @click="move(-1)">
            ...
            <template v-if="detail===keyNumber">
              <div class="grid"  @click="move(-1)">
                <h1 style="text-align: left; margin-bottom: 3px; margin-top: 3px; font-family: Pretendard-Regular,serif">
                  {{ key }}
                </h1>
                <h2 style="text-align: left; margin-top: 5px;flex: 1;">점수:{{ (report.report[key].score * 100) }}</h2>
                <svg fill="none" height="20" style="margin-top: 5px;" viewBox="0 0 12 20" width="20" xmlns="http://www.w3.org/2000/svg" @click="move(-1)">
  ...
  <div class="button-container">
    <div class="button" @click="getFrontendReport">프론트 분석 요청</div>
