...
const totalPoints = (props.data?.members || []).reduce((total, member) => {
  const memberPoint = (member.rating / 50) + 1;
...
<template>
  <Modal v-if="refModal" @closeModal="controlModal">
    <ModalTeam :data="data" :totalPoint="totalPoints"/>
  </Modal>
    ...
    <div class="recruiting-content">
      <div class="recruiting-text">
        <div>
          ...
          <span class="high-right">{{ props.data?.teamMember }}</span>
          <span class="normal">/ </span>
          <span class="normal">{{ props.data?.teamMaxMember }}</span>
        </div>
